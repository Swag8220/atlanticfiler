...
AddEventHandler('t1ger_drugs:intializeDrugs', function()
    if Job_NPC.entity ~= nil then DeleteEntity(Job_NPC.entity); Citizen.Wait(250) end
	-- Create NPC:
...
function EnterClosestDrugLab(id,val)
	...
	local data = {}
	ESX.TriggerServerCallback('t1ger_drugs:getLabCache', function(result) 
	...
	if recreate_lab then; data = CreateShellObject(id,val,true); while next(data) == nil do Citizen.Wait(100) end; can_enter = true end
	while not can_enter do Citizen.Wait(100) end
	-- Teleport:
...
function CreateShellObject(id,val,recreate)
	...
	while not NPC_Created do Citizen.Wait(10) end
	-- Sync Data:
	...
	if not recreate then 
		...
		ESX.TriggerServerCallback('t1ger_drugs:getLabCache', function(cache) 
...
function CreateLabPed(id,val,pos,heading)
	...
	if next(NPC_cfg.props) ~= nil then
			...
			if next(cur_lab.cache) then
				...
				if shell_dist <= 30.0 then
					...
					if process_dist <= 1.5 and not processing then
						...
						if next(cur_lab.cache.process) ~= nil and cur_lab.cache.process.done then draw_text = Config.DrugTypes[cur_lab.type].process.collect.text; collect = true end
						...
						if IsControlJustPressed(0, Config.DrugTypes[cur_lab.type].process.give.key) and not collect then 
							if next(cur_lab.cache.process) == nil then ProcessDrugs(cur_lab.cache) else TriggerEvent('t1ger_drugs:notify', Lang['already_processing']) end
...
function CollectProcessedDrugs(cache)
	ESX.TriggerServerCallback('t1ger_drugs:collectProcessedDrugs', function(collected) 
...
function ProcessDrugs(cache)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'drug_process_main',
		{
	...
	function(data, menu)
		if data.current.value == 'confirm' then
			...
			for i = 1, #menu.data.elements do
				if menu.data.elements[i].type == nil and menu.data.elements[i].name ~= nil then
		...
		if tonumber(data.current.value) and data.current.type == 'slider' then 
			...
			for i = 1, #elements do
				local newData = {}
				if elements[i].type == nil and elements[i].name ~= nil then
...
function PackageDrugs(cache)
	...
	table.insert(elements, {label = Lang['confirm_button'], value = 'confirm'})
	ESX.UI.Menu.CloseAll()
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'drug_package_main',
		{
	...
	function(data, menu)
		if data.current.value == 'confirm' then
			...
			ESX.TriggerServerCallback('t1ger_drugs:requestPackaging', function(can_package) 
				menu.close()
				if can_package then
		...
		if tonumber(data.current.value) and data.current.type == 'slider' then 
			...
			for i = 1, #elements do
				local newData = {}
				if elements[i].type == nil and elements[i].name ~= nil and elements[i].amount ~= 0 then
					newData.value = math.ceil(factor * elements[i].amount)
					newData.label = Lang['process_input_items']:format(newData.value, elements[i].name)
					menu.update({name = elements[i].name}, newData)
				end
...
function PlaySynchronizedScene(lab_cache)
	...
	for k,v in pairs(cfg.objects) do
		...
		while not DoesEntityExist(scene.obj[k]) do Citizen.Wait(0) end
	...
	-- Progress Bar
	if Config.ProgressBars then exports['progressBars']:startUI(cfg.time, cfg.text) end
	-- Wait Timer:
...
function DrugJobMainMenu()
	...
	if Config.ProgressBars then 
		exports['progressBars']:startUI((cfg.NPC.ply_anim.time * 1000), Lang['progbar_talking'])
	end
	Citizen.Wait(cfg.NPC.ply_anim.time * 1000)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'drug_job_main_menu',
		{
			title    = 'Vælg Drug Job',
			align    = 'center',
			elements = elements
	...
	function(data, menu)
		if isCop then ESX.UI.Menu.CloseAll(); interacting = false; return TriggerEvent('t1ger_drugs:notify', Lang['police_not_allowed']) end
        menu.close()
        ...
        if distance < 150 then
			...
			if veh_dist < 2.5 and not veh_lockpicked and not lockpicking then
				...
				if IsControlJustPressed(0, Config.DrugJobs.Settings.lockpick.key) then 
					...
					if Config.ProgressBars then 
						exports['progressBars']:startUI((Config.DrugJobs.Settings.lockpick.time * 1000), Lang['progbar_lockpicking'])
					end
					Citizen.Wait(Config.DrugJobs.Settings.lockpick.time * 1000)
					ClearPedTasks(player)
					FreezeEntityPosition(player, false)
					veh_lockpicked = true
					lockpicking = false
...
function SellDrugsToNPC(drug_ped, ped_pos)
	...
	ESX.TriggerServerCallback('t1ger_drugs:getUserInventory', function(inv_drugs, has_drugs)
		if has_drugs then
			ESX.TriggerServerCallback('t1ger_drugs:getPlayerMaxCap', function(maxCap)
				if not maxCap then
					...
					-- Progress Bars:
					if Config.ProgressBars then 
						exports['progressBars']:startUI(cfg.sell_time, Lang['progbar_selling'])
					end
					Citizen.Wait(cfg.sell_time)
					-- Chances:
					...
					Citizen.Wait(Config.DrugSale.wait_timer)
					selling_drugs = false
				else
...
RegisterCommand(Config.DrugSale.command, function(source, args)
	if can_sell_drugs then
		can_sell_drugs = false
		TriggerEvent('t1ger_drugs:notify', 'You disabled drug selling')
	else
		ESX.TriggerServerCallback('t1ger_drugs:getUserInventory', function(inv_drugs, has_drugs)
			if has_drugs then
				can_sell_drugs = true
				TriggerEvent('t1ger_drugs:notify', 'You enabled drug selling')
			else
				can_sell_drugs = false
				TriggerEvent('t1ger_drugs:notify', Lang['no_more_drugs'])
			end
...
AddEventHandler('t1ger_drugs:useDrugsCL', function(k,v)
	...
	if v.progressBar.enable then 
		exports['progressBars']:startUI(v.scenario.timer, v.progressBar.text)
	end
	Citizen.Wait(v.scenario.timer)
	ClearPedTasks(player)
...
AddEventHandler('t1ger_drugs:drugConverting', function(value)
	...
	if not IsPedInAnyVehicle(player) then
		...
		if Config.ProgressBars then
			exports['progressBars']:startUI(value.time, value.text)
		end
		Citizen.Wait(value.time)
		ClearPedTasks(player)
	else
		if Config.ProgressBars then
			exports['progressBars']:startUI(value.time, value.text)
