...
Config = {
    ...
    ESX_ItemLabel       = true,	                    -- set to false if your ESX vers. doesnt support item labels
    K4MB1_Labs          = true,                     -- only set to true if u own latest HIGH END DRUG LABS by K4MB1
    ...
    DrugSale = {
        ...
        min_cops        = 0,            -- required cops online to sell drugs to NPC's.
        ...
        command         = 'selldrugs'   -- command to enable/disable selling drugs (or fetch drug inventory)
    },
...
Config.DrugJobs = {
    ...
    NPC = {
        ...
        name        = 'Agrican',                     -- NPC name/number (if using phone msg)
        ...
        menu = {        
            ...
            [1] = { type = 'weed', label = 'Weed Job [$%s]', enable = true, fees = 1000, minCops = 0 },
            [2] = { type = 'meth', label = 'Meth Job [$%s]', enable = true, fees = 1250, minCops = 0 },
            [3] = { type = 'coke', label = 'Coke Job [$%s]', enable = true, fees = 1500, minCops = 0 }
        }
    },
    ...
    Reward = {
        ...
        ['weed'] = {
            [1] = { label = 'Weed Leaf', item = 'weed_leaf', chance = 100, amount = {min = 10, max = 20} }
        },
        ['meth'] = {
...
Config.DrugEffects = {
    ['coke_packaged'] = {                                                   -- item name  
        progressBar = { enable = true, text = "Ryger Crack kokain", },      -- progbar enable and progbar text
    ...
    ['meth_packaged'] = {                                                   -- item name  
        progressBar = { enable = true, text = "Ryger Meth", },            -- progbar enable and progbar text
        scenario = {name = 'WORLD_HUMAN_SMOKING_POT', timer = 5000},        -- scenario name and duration in ms
        duration = 30,                                                      -- effect duration seconds
        timeCycleMod = {enable = true, type = "spectator5"},                -- enable time cycle modifier and set type (https://pastebin.com/kVPwMemE)
    ...
    ['weed_joint'] = {                                                      -- item name  
        progressBar = { enable = true, text = "Ryger en joint", },            -- progbar enable and progbar text
        scenario = {name = 'WORLD_HUMAN_SMOKING_POT', timer = 5000},        -- scenario name and duration in ms
        duration = 30,                                                      -- effect duration seconds
        timeCycleMod = {enable = true, type = "spectator5"},                -- enable time cycle modifier and set type (https://pastebin.com/kVPwMemE)
        sprint = {enable = false, value = 1.2},                              -- enable faster sprint and set value (default 1.0)
...
Config.DrugLabs = {
    [1] = { pos = vector3(1223.52,-1495.16,34.99), h = 267.81, type = 'coke', shell = 'coke1', blip = true, cache = {} },   -- set pos, heading, type, shell, blip (enable/disable) for drug labs (don't touch 'cache')
    [2] = { pos = vector3(-1453.27,-387.31,38.19), h = 35.33, type = 'meth', shell = 'meth1', blip = true, cache = {} },  -- set pos, heading, type, shell, blip (enable/disable) for drug labs (don't touch 'cache')
    [3] = { pos = vector3(-182.79,220.01,88.82), h = 174.84, type = 'weed', shell = 'weed1', blip = true, cache = {} },  -- set pos, heading, type, shell, blip (enable/disable) for drug labs (don't touch 'cache')
    [4] = { pos = vector3(853.15,-2207.42,30.57), h = 263.88, type = 'coke', shell = 'coke2', blip = true, cache = {} },  -- set pos, heading, type, shell, blip (enable/disable) for drug labs (don't touch 'cache')
    [5] = { pos = vector3(-969.52,-266.42,38.55), h = 118.02, type = 'meth', shell = 'meth2', blip = true, cache = {} },  -- set pos, heading, type, shell, blip (enable/disable) for drug labs (don't touch 'cache')
    [6] = { pos = vector3(359.68,356.37,104.33), h = 342.53, type = 'weed', shell = 'weed2', blip = true, cache = {} },  -- set pos, heading, type, shell, blip (enable/disable) for drug labs (don't touch 'cache')
}
...
Config.DrugTypes = {
    ['coke'] = {
        sell = {
            ...
            multiplier = {enable = true, min_cops = 1, value = 10}
        },
        enter = { text = '~r~[E]~s~ Gå ind til Coke Laben', key = 38},             -- draw text & key to enter lab (DEFAULT [E])
        exit = { text = '~r~[E]~s~ Gå ud af Coke Laben', key = 38 },             -- draw text & key to exit lab (DEFAULT [E])
        process = {
            ...
            give        = {text = '~r~[G]~s~ Behandler kokain', key = 47},          -- draw text & key to process/give (DEFAULT [G])
            collect     = {text = '~r~[E]~s~ Indsamler Skåret Kokain', key = 38},   -- draw text & key to collect (DEFAULT [E])
            input = {
                [1] = {label = 'Kokain Blad', item = 'coca_leaf', amount = 5},    -- input/required item, label and amount to process output.amount.min
                [2] = {label = 'Chemicals', item = 'chemicals', amount = 2}     -- input/required item, label and amount to process output.amount.min
            },
            output = {
                label   = 'Skåret Kokain',    -- output/reward item label
        ...
        package = {
            text                = '~r~[E]~s~ Pakker Kokain',     -- draw text to package
    ...
    ['meth'] = {
        sell = {
            ...
            multiplier = {enable = true, min_cops = 2, value = 10}
        },
        enter = { text = '~r~[E]~s~ Gå ind til  Meth Laben', key = 38},             -- draw text & key to enter lab (DEFAULT [E])
        exit = { text = '~r~[E]~s~ Gå ud af Meth Lab', key = 38 },             -- draw text & key to exit lab (DEFAULT [E])
        process = {
            ...
            give        = {text = '~r~[G]~s~ Behandler Meth', key = 47},          -- draw text & key to process/give (DEFAULT [G])
            collect     = {text = '~r~[E]~s~ Indsamler Ren Meth', key = 38},   -- draw text & key to collect (DEFAULT [E])
            input = {
            ...
            output = {
                label   = 'Ren Meth',    -- output/reward item label
        ...
        package = {
            ...
            input = {
                ...
                [2] = {label = 'Plastik Meth poser', item = 'plastic_bags', amount = 1}   -- input/required item, label and amount to package output.amount.min
            },
            output = {
                label           = 'Pakket Meth',      -- output/reward item label
    ...
    ['weed'] = {
        sell = {
            items = {
                ['weed_packaged'] = {min = 150, max = 400},                 -- item name, min & max sell price
                -- add more sellable coke items in here if u want to.
            },
            multiplier = {enable = false, min_cops = 0, value = 10}
        },
        enter = { text = '~r~[E]~s~ Gå ind til Weed Laben', key = 38},             -- draw text & key to enter lab (DEFAULT [E])
        exit = { text = '~r~[E]~s~ Gå ud af Weed Laben', key = 38 },             -- draw text & key to exit lab (DEFAULT [E])
        process = {
            ...
            output = {
                label   = 'Ubeskåret Hash',     -- output/reward item label
        ...
        package = {
            ...
            key                 = 38,                           -- key to package (DEFAULT [E])
            input = {
                [1] = {label = 'Ubeskåret Joints', item = 'weed_untrimmed', amount = 1},  -- input/required item, label and amount to package output.amount.min
                [2] = {label = 'Plastik Joints Poser', item = 'plastic_bags', amount = 1}       -- input/required item, label and amount to package output.amount.min
            },
            output = {
                label           = 'Pakket Joints',      -- output/reward item label
...
Config.DrugItemConverter = {
    -- ROLLING JOINTS:
    ['weed_packaged'] = {                                                   -- usable item name
        ...
        text = 'Ruller Joints',                                             -- progressbars text
