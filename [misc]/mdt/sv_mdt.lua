...
AddEventHandler("mdt:getOffenderDetailsById", function(char_id)
	...
	if not offender then
		...
		TriggerClientEvent("mdt:sendNotification", usource, "Denne person findes ikke længere")
		return
	end
...
AddEventHandler("mdt:saveOffenderChanges", function(id, changes, identifier)
	...
	MySQL.Async.fetchAll('SELECT * FROM `user_mdt` WHERE `char_id` = @id', {
		...
		TriggerClientEvent("mdt:sendNotification", usource, "Ændringerne er blevet gemt")
	end)
end)
...
AddEventHandler("mdt:saveReportChanges", function(data)
	...
	TriggerClientEvent("mdt:sendNotification", source, "Rapport ændreinger er blevet gemt")
end)
...
AddEventHandler("mdt:deleteReport", function(id)
	...
	TriggerClientEvent("mdt:sendNotification", source, "Rapporten er blevet slettet")
end)
...
AddEventHandler("mdt:submitNewReport", function(data)
	...
	MySQL.Async.insert('INSERT INTO `mdt_reports` (`char_id`, `title`, `incident`, `charges`, `author`, `name`, `date`, `jailtime`) VALUES (@id, @title, @incident, @charges, @author, @name, @date, @sentence)', {
		...
		TriggerClientEvent("mdt:sendNotification", usource, "Der er indsendt en ny rapport.")
	end)
			...
			MySQL.Async.fetchAll('SELECT * FROM `characters` WHERE `identifier` = @identifier', {
				...
				if result[1].id == char_id then
					if jailtime and jailtime > 0 then
						...
						TriggerEvent("esx2-qalle-jail:jailPlayer", src, jailtime, jailmsg)
					end
					if fine > 0 then
...
AddEventHandler("mdt:submitNewWarrant", function(data)
	...
	MySQL.Async.insert('INSERT INTO `mdt_warrants` (`name`, `char_id`, `report_id`, `report_title`, `charges`, `date`, `expire`, `notes`, `author`) VALUES (@name, @char_id, @report_id, @report_title, @charges, @date, @expire, @notes, @author)', {
		...
		TriggerClientEvent("mdt:sendNotification", usource, "En ny efterlysning er blevet oprettet")
	end)
end)
...
AddEventHandler("mdt:deleteWarrant", function(id)
	...
	TriggerClientEvent("mdt:sendNotification", usource, "Efterlysningen er blevet slettet.")
end)
...
AddEventHandler("mdt:getReportDetailsById", function(query, _source)
	...
	MySQL.Async.fetchAll("SELECT * FROM `mdt_reports` WHERE `id` = @query", {
		...
		if result and result[1] then
			...
			TriggerClientEvent("mdt:closeModal", usource)
			TriggerClientEvent("mdt:sendNotification", usource, "Denne rapport kan ikke blive fundet")
		end
	end)
end)
...
AddEventHandler("mdt:newCall", function(details, caller, coords)
	...
	for i= 1, #xPlayers do
		...
		if xPlayer.job.name == 'police' then
			...
			TriggerEvent("InteractSound_SV:PlayOnOne", source, "demo", 0.2)
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = "Du har modtaget et nyt opkald.", length = '5000', style = {}})
...
AddEventHandler("mdt:attachToCall", function(index)
	...
	TriggerClientEvent("mdt:sendNotification", usource, "Du er nu tilknyttet dette opkald.")
end)
...
AddEventHandler("mdt:detachFromCall", function(index)
	...
	TriggerClientEvent("mdt:sendNotification", usource, "Du er fjernet fra dette opkald.")
end)
...
AddEventHandler("mdt:editCall", function(index, details)
	...
	TriggerClientEvent("mdt:sendNotification", usource, "Du har redigeret dette opkald")
end)
...
AddEventHandler("mdt:deleteCall", function(index)
	...
	TriggerClientEvent("mdt:sendNotification", usource, "Du har slettet dette opkald")
end)
...
AddEventHandler("mdt:saveVehicleChanges", function(data)
	...
	MySQL.Async.fetchAll('SELECT * FROM `vehicle_mdt` WHERE `plate` = @plate', {
		...
		TriggerClientEvent("mdt:sendNotification", usource, "Køretøj's ændreinger er blevet ændret")
	end)
end)
