...
RegisterNetEvent("atlantic-opkaldsliste")
AddEventHandler("atlantic-opkaldsliste", function()
    RegisterNUICallback("getCalls", function(data, cb)
        TriggerServerEvent("mdt:getCalls")
    end)
end)
...
RegisterNUICallback("attachToCall", function(data, cb)
    ...
    if not callBlip then
        ...
        AddTextComponentString("112 Opkald")
...
AddEventHandler("mdt:billPlayer", function(src, sharedAccountName, label, amount)
    TriggerServerEvent("esx2_billing:serverloggingsendBill", src, sharedAccountName, label, amount)
end)
