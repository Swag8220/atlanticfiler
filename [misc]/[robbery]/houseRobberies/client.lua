...
AddEventHandler('houseRobberies:attempt', function(lockpicks)
 ...
 if lockpicks > 0  and not isRobbing then
  ...
  for id,v in pairs(robbableHouses) do
   if GetDistanceBetweenCoords(playerCoords, v.x, v.y, v.z, true) <= 2.5 then
    ...
    exports["t0sic_loadingbar"]:StartDelayedFunction("Bryder ind i huset", 12000, function()
     isLockpicking = false
     pedSpawned = false
 ...
 elseif lockpicks == 0 and not isRobbing then
  ...
  TriggerEvent('notification', 'Intet koben', 2)
 else
  TriggerEvent('notification', "Du kan ikke røve huset", 2)
 end
end)
...
AddEventHandler('houseRobberies:createHouse', function(id)
 ...
 while isRobbing do
  ...
  if GetDistanceBetweenCoords(playerCoords, house.x+3.6, house.y-15, house.z-50, true) < 2.5 then
   showMessage('Tryk ~g~E~w~ for at forlade huset')
...
RegisterNetEvent('houseRobberies:logging')
AddEventHandler('houseRobberies:logging', function()
  ...
  if isRobbing then
   for i=1,#myRobbableItems do
    if (GetDistanceBetweenCoords(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"], generator.z + myRobbableItems[i]["z"], GetEntityCoords(GetPlayerPed(-1))) < 1.4) and not myRobbableItems[i]['isSearched'] then
     DrawText3Ds(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"], generator.z + myRobbableItems[i]["z"], '~w~Tryk på ~g~H~s~ for og søge ' .. myRobbableItems[i]["name"])
     ...
     if IsControlJustReleased(1, 74) then
      ...
      exports["t0sic_loadingbar"]:StartDelayedFunction("Søger "..myRobbableItems[i]['name'], 20000, function()
