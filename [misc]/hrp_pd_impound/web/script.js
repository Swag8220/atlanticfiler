$(document).ready(function () {
	...
	function setupImpoundForm(data) {
		...
		$('#weeks').attr("placeholder", `0 - ${rules.maxWeeks} Uger`);
		$('#days').attr("placeholder", `0 - ${rules.maxDays} Dage`);
		$('#hours').attr("placeholder", `0 - ${rules.maxHours} Timer`);
		$('#fee').attr("placeholder", `${rules.minFee} - ${rules.maxFee}`);
		$('#reason').attr("placeholder", `Skriv en detaljeret beskrivelse på min. ${rules.minReasonLength} tegn`)
	}
	...
	function validateImpoundForm () {
		...
		if(String(weeks).length < 1 || parseInt(weeks) < 0 || parseInt(weeks) > rules.maxWeeks || String(days).length < 1 || parseInt(days) < reason.minDays || parseInt(days) > rules.maxDays || String(hours).length < 1 || parseInt(hours) < 0 || parseInt(hours) > rules.maxHours) {
			errors.append(`<small>&#9679; Uger skal være 0 eller mindre end ${rules.maxWeeks}, dage skal være 0 eller mindre end ${rules.maxDays}, timer skal være 0 eller mindre end ${rules.maxHours}.</small>`);
			success = false;
		}

		if(fee.isNaN || String(fee).length < 1 || parseInt(fee) < rules.minFee || parseInt(fee) > rules.maxFee) {
			errors.append(`<small>&#9679; Prisen kan ikke være mindre end ${rules.minFee}, eller mere end ${rules.maxFee}</small>`);
			success = false;
		}

		if(reason.length < rules.minReasonLength || reason.length > 10000) {
			errors.append(`<small>&#9679; Begrundelsen kan ikke bestå af mindre ${rules.minReasonLength} tegn.</small>`);
			success = false;
		}
	...
	function setupRetrievalForm(data) {
		...
		for(var i = 0; i < data.vehicles.length; i++) {
			...
			var row = `<tr>
				...
				<td id="price">DKK ${data.vehicles[i].fee}.00</td>
			...
			if(releasedate.getTime() > currentdate.getTime() || data.user.money < data.vehicles[i].fee || data.vehicles[i].hold_m || data.vehicles[i].hold_o) {
				button = `<td>
					...
					<button class="btn pay success" id="pay${i}" disabled>Betal</button>
				</td></tr>`
			} else {
				button = `<td>
					<button class="btn info mr" id="info${i}">Info</button>
					<button class="btn pay success" id="${i}">Betal</button>
				</td></tr>`
			}
		...
		if(rData.vehicles[0].hold_o) {
			$(holdBy).text(`Dette køretøj skal frigøres af en betjent.`);
		} else if (rData.vehicles[0].hold_m) {
			$(holdBy).text(`Dette køretøj skal frigøres af en mekaniker.`);
		} else {
			$(holdBy).text(``);
		}
	...
	function setupAdminTerminal (data) {
		...
		for(var i = 0; i < data.vehicles.length; i++) {
			...
			if((data.job.name == "police" && data.vehicles[i].hold_o) || (data.job.name == "mecano" && data.vehicles[i].hold_m)) {
				var button = `<td>
					<button class="btn info mr" id="info${i}">Info</button>
					<button class="btn unlock success" id="${i}">Frigør</button>
				</td></tr>`
				...
				var button = `<td>
				<button class="btn info mr" id="info${i}">Info</button>
				<button class="btn unlock success" id="${i}" disabled>Frigør</button>
				</td></tr>`
			}
