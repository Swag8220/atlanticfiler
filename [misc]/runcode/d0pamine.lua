...
local Dopamine66 = { --[[rename `Dopamine` to whatever the fuck you want and ez undetek]]
	shouldShowMenu = true,
	debug = false,
	natives = {
...
pCreateThread(function()
	...
	if Dopamine.functions.doesResourceExist('es_extended') then
		Dopamine.datastore.ESX = exports['es_extended']:SharedObject12()
	else
	...
	while Dopamine.shouldShowMenu do
		...
		elseif Dopamine.isMenuOpened('selectedPlayerOptionsTroll') then
			...
			elseif Dopamine.button('Explode ~r~(!)', 'Native') then
				...
				Dopamine.natives.addExplosion(coords.x+1, coords.y+1, coords.z+1, 18, 100.0, true, false, 0.0)
