...
--[[ Main Event Handlers ]]--
    ...
    AddEventHandler("K9:OpenMenu", function(pedRestriction, pedList)
        if pedRestriction then
            if CheckPedRestriction(GetLocalPed(), pedList) then
                ...
                Notification(tostring("~r~Niksen karen bliksen."))
            end
        else
            EnableMenu()
        end
    ...
    AddEventHandler("K9:IdentifierRestricted", function()
        Notification(tostring("~r~Hvad tror du selv?"))
    end)
    ...
    AddEventHandler("K9:SearchVehicle", function(items, openDoors)
        ...
        if vehicle ~= 0 and not searching then
            ...
            Notification(tostring(k9_name .. " er begyndt at søge..."))
            ...
            Notification(tostring(k9_name .. " har fundet [ " .. tostring(table.concat(stringified_table, ", ")) .. " ]."))
            searching = false
        end
    end)
...
--[[ Threads ]]
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            -- Trigger Opens Menu
            if IsControlPressed(1, 19) and IsControlJustPressed(1, 311) then
