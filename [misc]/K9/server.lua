...
--    if not vRP.hasGroup({user_id,"HarEnHund"}) then vRP
--        TriggerClientEvent("pNotify:SendNotification", player,{text = "Du har ikke nogle Hund", type = "error", queue = "global", timeout = 10000, layout = "centerRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
--    else
    local src = source
