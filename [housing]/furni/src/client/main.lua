            ...
            if ePos.x ~= 0.0 then
              ...
              if not furni.antiGrab[model] then
                if wasPressing and keyHeldFor >= 500 then
                  Utils.drawText3D(pos,"~r~Frigøre [~s~LMB/RT~r~] at skifte [~s~"..(hashtable[model] and hashtable[model].name or "Unknown").."~r~]~s~",1,7)
                else
                  Utils.drawText3D(pos,"~y~Hold [~s~LMB/RT~y~] at skifte [~s~"..(hashtable[model] and hashtable[model].name or "Unknown").."~y~]~s~",1,7)
                end
        ...
        elseif furni.doingRemoval then
          ...
          if ent and ent ~= -1 and ent ~= 0 then
            ...
            if ePos and ePos.x and ePos.x ~= 0 and ePos.x ~= 0.0 then
              local model = GetEntityModel(ent)
              if not furni.antiGrab[model] then
                Utils.drawText3D(pos,"~y~Hold [~s~LMB/RT~y~] For at sælge [~s~"..(hashtable[model] and hashtable[model].name or "Unknown").."~y~] [~s~"..(hashtable[model] and "$"..math.floor(hashtable[model].price*(Config.ResaleValue and Config.ResaleValue/100.0 or 0.5)) or "Unknown").."~y~]~s~",1,7)
                if IsDisabledControlPressed(0, Controls["Grab"]) or IsControlPressed(0, Controls["Grab"]) and not furni.focused then
                  keyHeldFor = keyHeldFor + (GetGameTimer() - lastFrame)
                  if keyHeldFor >= 500 then
                    keyHeldFor = 0
  ...
  if furni.inHouse then
    ...
    if furni.wasEditing then
      ...
      if CanPlayerAfford(item.price) then
        ...
        Utils.showNotification("Du har ikke råd til denne vare.")
        SetEntityAsMissionEntity(furni.object,true,true)
        DeleteObject(furni.object)
      end
    end
...
furni.openCommand = function(s,args) 
  if furni.inHouse then
    ...
    Utils.showNotification("~r~Du skal være inde i et hus.")
  end
end
