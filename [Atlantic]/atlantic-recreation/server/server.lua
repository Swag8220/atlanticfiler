...
RegisterServerEvent('Atlantic_recreation:bikerental')
AddEventHandler('Atlantic_recreation:bikerental', function(status)
    local Source = source
    local identifier = GetPlayerIdentifiers(Source)[1]
end)


ESX.RegisterServerCallback('Atlantic_recreation:bikerental', function(source, cb, status)
    local Source = source
    local identifier = GetPlayerIdentifiers(Source)[1]
...
ESX.RegisterServerCallback('Atlantic_recreation:payment', function(source, cb, zone, itemId, itemType)
