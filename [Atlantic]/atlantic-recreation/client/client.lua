...
Citizen.CreateThread(function()
    ...
    ESX.TriggerServerCallback('Atlantic_recreation:bikerental', function(status)
        RentedBike = status
        TriggerEvent('esx:removeWeapon', 'GADGET_PARACHUTE')
    end, "check")
end)

AddEventHandler('Atlantic_recreation:hasEnteredMarker', function(zone)
    CurrentAction = MenuActions.MenuTrigger
    CurrentActionMsg = MenuActions.MenuMessage
    CurrentActionData = {zone = zone}
end)

AddEventHandler('Atlantic_recreation:hasExitedMarker', function(zone)
    CurrentAction = nil
...
Citizen.CreateThread(function()
    while true do
        ...
        if isInMarker and not HasAlreadyEnteredMarker then
            ...
            TriggerEvent('Atlantic_recreation:hasEnteredMarker', currentZone)
        end
        if not isInMarker and HasAlreadyEnteredMarker then
            HasAlreadyEnteredMarker = false
            TriggerEvent('Atlantic_recreation:hasExitedMarker', LastZone)
        end
...
function OpenMenu(zone)
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'multimenu', {
        ...
        if data.current.action == 'bikerental_menu' then
            ...
            ESX.TriggerServerCallback('Atlantic_recreation:payment', function(payed)
                if payed then
