...
function takeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("Du har brug for en pistol for at tage et gidsel!")
	end
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		if closestPlayer ~= -1 and closestPlayer ~= nil then
			...
			drawNativeNotification("Ingen spiller i nærheden!")
		end 
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"Tryk [G] for at give slip, [H] for at dræbe")
