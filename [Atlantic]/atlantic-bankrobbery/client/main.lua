	...
	for k,v in pairs(cfg.items) do
		if math.random(0,100) <= v.chance then
			...
			if invItem ~= nil then
				...
				print("^1[Iten Fejl] - ["..string.upper(item).."] Findes ikke i databasen^0")
			end
		end
		Citizen.Wait(10)
	end
	...
	for i=1, #xPlayers, 1 do
		...
		for k,v in pairs(Config.PoliceJobs) do
			if xPlayer.job.name == v then
				TriggerClientEvent('chatMessage', xPlayers[i], "^Politi information: | ^7", { 9, 41, 69, 1 }, string.sub('Banken er sikret. Alle banker er nu åbne igen!',0))
			end
		end
	end
end)
