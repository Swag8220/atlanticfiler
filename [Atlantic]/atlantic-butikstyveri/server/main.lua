	...
	if player then
		for safeStore, safeData in pairs(cachedSafes) do
			if safeData["saveData"] then
				...
				-- TriggerEvent("mdt:newCall", "Igangværende butikstyveri nær: ", vector3(playerCoords.x, playerCoords.y, playerCoords.z))
			end
		end
	...
	if player then
		if reward then
			if reward == "money" then
				...
				TriggerClientEvent("esx:showNotification", source, "Du stjal DKK" .. rewardAmount)
--			else
