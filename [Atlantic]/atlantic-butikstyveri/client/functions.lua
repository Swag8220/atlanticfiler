...
TryToCrack = function(safeEntity)
	...
	ESX.TriggerServerCallback("safebreach:checkIfSafeIsBreachable", function(state)
		...
		TriggerServerEvent('esx_outlawalert:robberyInProgress', {
			...
			ESX.ShowNotification("Der er ikke nok betjente.")
		end
		...
		while isCracking do	
			...
			if IsControlJustPressed(0, 22) then
				if dialRotation == currentSafeCorrectPosition then
					...
					if #safeLocks < Config.TotalLocks then
						...
						ESX.TriggerServerCallback("safebreach:safeBreached", function(breached)
							if breached then
								...
								ESX.ShowNotification("Du åbnede pengeskabet!")
							else
								ESX.ShowNotification("Pengeskabet er allerede åbent!")
							end
						end, {
							["store"] = currentStreetName,
							["doorCoords"] = {
					...
					ESX.ShowNotification("Forkert kombination, prøv igen.")
				end
...
TakeSafeItem = function(itemEntity, safeData)
	...
	if DoesEntityExist(itemEntity) then
		...
		local stoleItem = GetEntityModel(itemEntity) == GetHashKey("prop_anim_cash_pile_02") and "money"
		...
		ESX.ShowNotification("En person stjal før dig!!")
	end
end
