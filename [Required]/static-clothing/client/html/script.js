...
$(function () {
    ...
    $("#saveoutfit").click(function() {
        $("#savenewoutfitname_input").val("");
        $('.button-menu').removeClass('active')
        $('.button-menu').each(function() {
            $("#" + $(this).attr('data-target')).fadeOut(100);
        })
        $("#outfitoptions").fadeOut(100);
        $("#changeoutfitname").fadeOut(100);

        let t = $("#savenewoutfitname")
        t.fadeIn(100);
    });

    $("#outfitoptions_back").click(function() {
        $('.button-menu').removeClass('active')
        $('.button-menu').each(function() {
            $("#" + $(this).attr('data-target')).fadeOut(100);
        })
        $("#savenewoutfitname").fadeOut(100);
        $("#outfitoptions").fadeOut(100);
        $("#changeoutfitname").fadeOut(100);

        let t = $("#outfits")
        $("#button-outfits").addClass('active');
        t.fadeIn(100);
    });

    $("#savenewoutfitname_back").click(function() {
        $('.button-menu').removeClass('active')
        $('.button-menu').each(function() {
            $("#" + $(this).attr('data-target')).fadeOut(100);
        })
    ...
    $("#savenewoutfitname_submit").click(function() {
        $.post('http://static-clothing/savenewoutfit', JSON.stringify({name:$("#savenewoutfitname_input").val()}));
    });

    $("#outfitoptions_anvend").click(function() {
        $.post('http://static-clothing/useoutfit', JSON.stringify({outfit:window.CurrentOutfitChosen}));
    });

    $("#outfitoptions_overskriv").click(function() {
        $.post('http://static-clothing/overrideoutfit', JSON.stringify({outfit:window.CurrentOutfitChosen}));
    });

    $("#outfitoptions_rename").click(function() {
        $("#changeoutfitname_input").val('');
        $('.button-menu').removeClass('active')
        $('.button-menu').each(function() {
            $("#" + $(this).attr('data-target')).fadeOut(100);
        })
        ...
        let t = $("#changeoutfitname")
        t.fadeIn(100);
    });

    $("#outfitoptions_slet").click(function() {
        $.post('http://static-clothing/deleteoutfit', JSON.stringify({outfit:window.CurrentOutfitChosen}));
    });

    $("#changeoutfitname_back").click(function() {
        $('.button-menu').removeClass('active')
        $('.button-menu').each(function() {
            $("#" + $(this).attr('data-target')).fadeOut(100);
        })
    ...
    $("#changeoutfitname_submit").click(function() {
        $.post('http://static-clothing/renameoutfit', JSON.stringify({outfit:window.CurrentOutfitChosen,name:$("#changeoutfitname_input").val()}));
    });
    ...
    window.addEventListener('message', function (event) {
        if (event.data.type == "enableclothesmenu") {
            ...
            if (open) {
                ...
                setTimeout(function () {
                    ...
                    if ($("#savenewoutfitname").css("display") != "none" || $("#outfitoptions").css("display") != "none" || $("#changeoutfitname").css("display") != "none") {
                        $('.button-menu').removeClass('active')
                        $('.button-menu').each(function() {
                            $("#" + $(this).attr('data-target')).fadeOut(100);
                        })
        ...
        if (event.data.type == "outfits") {
            window.outfits = event.data.data;
            $(".saved-outfit").remove();
            var i = 0;
            for (const key in outfits) {
                if (Object.hasOwnProperty.call(outfits, key)) {
                    $(".all-outfits").append(`<button class="saved-outfit saved-outfit-${i} row waves-effect waves-light btn outfitbutton">${key}</button>`);
                    $(`.saved-outfit-${i}`).click(function() {
                        window.CurrentOutfitChosen = Object.keys(window.outfits)[Number($(this).attr("class").split(" ")[1].split("-")[2])];
                        $('.button-menu').removeClass('active')
                        $('.button-menu').each(function() {
                            $("#" + $(this).attr('data-target')).fadeOut(100);
                        })
    ...
    function CloseMenu(save) {
        $.post('http://static-clothing/escape', JSON.stringify({save:save}));
    }

    $(document).on('contextmenu', function() {
        $.post('http://static-clothing/togglecursor', JSON.stringify({}));
    })
    ...
    $('#skin_string').find('input').keypress(function (e) {
        if (e.which == 13) {
            $.post('http://static-clothing/customskin', JSON.stringify($(this).val()));
        }
    })
    ...
    function inputChange(ele,inputType) {
        ...
        if (currentMenu.is($('#clothesmenu')) || $(ele).parents('.panel').hasClass('hair')) {
            ...
            if ($(ele).parents('.panel').attr('id') == "skins") {
                $.post('http://static-clothing/setped', JSON.stringify({
                    "name": $(ele).attr('id'),
                    "value": $(ele).val()
                }))
            }
                ...
                $.post('http://static-clothing/updateclothes', JSON.stringify({
                    "name": nameId,
            ...
            $.post('http://static-clothing/settats', JSON.stringify({tats}))
        }
    }
    ...
    $('.tog_head').on('click', function() {
        toggleCam($(this));
        $.post('http://static-clothing/switchcam', JSON.stringify({name: 'head'}))
    })
    $('.tog_torso').on('click', function() {
        toggleCam($(this));
        $.post('http://static-clothing/switchcam', JSON.stringify({name: 'torso'}))
    })
    $('.tog_leg').on('click', function() {
        toggleCam($(this));
        $.post('http://static-clothing/switchcam', JSON.stringify({name: 'leg'}))
    })
    $('.tog_cam').on('click', function() {
        toggleCam($(this));
        $.post('http://static-clothing/switchcam', JSON.stringify({name: 'cam'}))
    })


    $('.tog_hat').on('click', function() {
        $.post('http://static-clothing/toggleclothes', JSON.stringify({name: "hats"}))
    })
    $('.tog_glasses').on('click', function() {
        $.post('http://static-clothing/toggleclothes', JSON.stringify({name: "glasses"}))
    })
    $('.tog_tops').on('click', function() {
        // dont look at this :)
        $.post('http://static-clothing/toggleclothes', JSON.stringify({name: "jackets"}))
        $.post('http://static-clothing/toggleclothes', JSON.stringify({name: "undershirts"}))
        $.post('http://static-clothing/toggleclothes', JSON.stringify({name: "torsos"}))
        $.post('http://static-clothing/toggleclothes', JSON.stringify({name: "vest"}))
        $.post('http://static-clothing/toggleclothes', JSON.stringify({name: "bags"}))
        $.post('http://static-clothing/toggleclothes', JSON.stringify({name: "neck"}))
        $.post('http://static-clothing/toggleclothes', JSON.stringify({name: "decals"}))
    })
    $('.tog_legs').on('click', function() {
        $.post('http://static-clothing/toggleclothes', JSON.stringify({name: "legs"}))
    })
    $('.tog_mask').on('click', function() {
        $.post('http://static-clothing/toggleclothes', JSON.stringify({name: "masks"}))
    })

    $('#reset').on('click', function() {
        $.post('http://static-clothing/resetped', JSON.stringify({}))
    })
    ...
    window.addEventListener("keydown", throttle(function (ev) {
        ...
        if (_key) {
            if (!$("#savenewoutfitname_input").is(":focus") && !$("#changeoutfitname_input").is(":focus")) {
                $.post('http://static-clothing/rotate', JSON.stringify({key: _key}))
            }
        }
    }, 50))
    ...
    function SaveHeadBlend() {
        ...
        $.post('http://static-clothing/saveheadblend', JSON.stringify(headBlend))
    }

    function SaveFaceShape(ele) {
        $.post('http://static-clothing/savefacefeatures', JSON.stringify({name: ele.attr('data-value'), scale: ele.val()}))
    }
    ...
    function SaveHeadOverlay(ele) {
        ...
        $.post('http://static-clothing/saveheadoverlay', JSON.stringify({
            name: id,
            value: inputs.eq(0).val(),
            opacity: opacity
        }))
    ...
    function AddPalettes() {
        ...
        $('.color_palette').on('click', function() {
            ...
            if ($(this).parents('.panel').hasClass('hair')) {
                $.post('http://static-clothing/savehaircolor', JSON.stringify({
                    firstColour: palettes.eq(0).find('.active').attr('value'),
                    secondColour: palettes.eq(1).find('.active').attr('value')
                }));
            }
            else {
                $.post('http://static-clothing/saveheadoverlaycolor', JSON.stringify({
                    firstColour: palettes.eq(0).find('.active').attr('value'),
                    secondColour: palettes.eq(1).find('.active').attr('value'),
