...
ESX.RegisterServerCallback('esx_advancedgarage:checkMoneyAmbulance', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getAllMoney() >= Config.Ambulance.PoundP then
		cb(true)
	else
		cb(false)
	end
...
AddEventHandler('esx_advancedgarage:payAmbulance', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	xPlayer.removeAllMoney(Config.Ambulance.PoundP)
...
ESX.RegisterServerCallback('esx_advancedgarage:checkMoneyPolice', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getAllMoney() >= Config.Police.PoundP then
		cb(true)
	else
		cb(false)
	end
...
AddEventHandler('esx_advancedgarage:payPolice', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	xPlayer.removeAllMoney(Config.Police.PoundP)
...
ESX.RegisterServerCallback('esx_advancedgarage:checkMoneyMechanic', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getAllMoney() >= Config.Mechanic.PoundP then
		cb(true)
	else
		cb(false)
	end
...
AddEventHandler('esx_advancedgarage:payMechanic', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	xPlayer.removeAllMoney(Config.Mechanic.PoundP)
...
ESX.RegisterServerCallback('esx_advancedgarage:checkMoneyAircrafts', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getAllMoney() >= Config.Aircrafts.PoundP then
		cb(true)
	else
		cb(false)
	end
...
AddEventHandler('esx_advancedgarage:payAircraft', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	xPlayer.removeAllMoney(Config.Aircrafts.PoundP)
...
ESX.RegisterServerCallback('esx_advancedgarage:checkMoneyBoats', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getAllMoney() >= Config.Boats.PoundP then
		cb(true)
	else
		cb(false)
	end
...
AddEventHandler('esx_advancedgarage:payBoat', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	xPlayer.removeAllMoney(Config.Boats.PoundP)
...
ESX.RegisterServerCallback('esx_advancedgarage:getOwnedCars', function(source, cb)
	...
	if Config.Main.ShowVehLoc then
		print("fuck da dig")
		MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job AND storedhouse = false AND owned_vehicles.plate NOT IN (SELECT plate from h_impounded_vehicles); ', { -- job = NULL
			['@owner'] = xPlayer.identifier,
			['@Type'] = 'car',
			['@job'] = 'civ'
		}, function(data)
		...
		print("her")
		MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job AND `stored` = @stored AND owned_vehicles.plate NOT IN (SELECT plate from h_impounded_vehicles);', { -- job = NULL
			['@owner'] = xPlayer.identifier,
			['@Type'] = 'car',
			['@job'] = 'civ',
			['@stored'] = true
...
ESX.RegisterServerCallback('esx_advancedgarage:checkMoneyCars', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getAllMoney() >= Config.Cars.PoundP then
		cb(true)
	else
		cb(false)
	end
...
AddEventHandler('esx_advancedgarage:payCar', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	xPlayer.removeAllMoney(Config.Cars.PoundP)
...
ESX.RegisterServerCallback('esx_advancedgarage:storeVehicle', function (source, cb, vehicleProps)
	local ownedCars = {}
	local vehplate = vehicleProps.plate:match("^%s*(.-)%s*DKK gå til mekaniker")
