	...
	ESX.TriggerServerCallback('esx_advancedgarage:getOwnedCars', function(ownedCars)
		if #ownedCars == 0 then
			...
			for _,v in pairs(ownedCars) do
				...
				local vehicleName = GetLabelText(aheadVehName)
				local plate = v.plate
				local health = v.vehicle.health
				if health == nil then
					health = 1000
				else
					health = health
				end
				local vehDamage	   = math.floor(health/10)
				local labelvehicle
				local labelvehicle2 = ('| <span style="color:red;">%s</span> - <span style="color:darkgoldenrod;">%s</span> | '):format(plate, vehicleName)..' <span style="color:darkgoldenrod;"> '..vehDamage..'%</span>  '
				local labelvehicle2 = ('| <span style="color:red;">%s</span> - <span style="color:darkgoldenrod;">%s</span> | '):format(plate, vehicleName)..' <span style="color:darkgoldenrod;"> '..vehDamage..'%</span>  '
...
function RepairVehicle(apprasial, vehicle, vehicleProps)
	...
	local elements = {
		{label = _U('return_vehicle').." (DKK"..apprasial..")", value = 'yes'},
