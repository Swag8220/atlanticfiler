...
AddEventHandler('es:incorrectAmountOfArguments', function(source, wantedArguments, passedArguments, user, command)
	if(source == 0)then
		...
		TriggerClientEvent('chat:addMessage', source, {
			args = {"", "Incorrect amount of arguments! (" .. passedArguments .. " passed, " .. requiredArguments .. " wanted)"}
		})
	end
end)
				...
				TriggerEvent('es:canGroupTarget', user.getGroup(), target.getGroup(), function(canTarget)
					if canTarget and available then
						...
						if not available then
							TriggerClientEvent('chat:addMessage', Source, {
								args = {"", "Du har ikke adgang til denne funktion!"}
							})
						else
							TriggerClientEvent('chat:addMessage', Source, {
								args = {"", "Du har ikke adgang til denne funktion!"}
	...
	TriggerEvent('es:getPlayerFromId', source, function(user)
		TriggerEvent('es:canGroupTarget', user.getGroup(), "admin", function(available)
						...
						if(groups[GROUP])then
							TriggerEvent("es:setPlayerData", USER, "group", GROUP, function(response, success)
								...
								TriggerClientEvent('chat:addMessage', -1, {
									args = {"", "Ranken for ^2^*" .. GetPlayerName(tonumber(USER)) .. "^r^0 Er blevet sat til ^2^*" .. GROUP}
								})
							end)
						else
							TriggerClientEvent('chat:addMessage', Source, {
								args = {"", "Ranken Blev ikke fundet!"}
							})
						end
					end)
				end
				...
				if(GetPlayerName(USER) == nil)then
					...
					if(GROUP ~= nil and GROUP > -1)then
						TriggerEvent("es:setPlayerData", USER, "permission_level", GROUP, function(response, success)
							if(true)then
								TriggerClientEvent('chat:addMessage', -1, {
									args = {"", "Tilladelsesniveau for ^2" .. GetPlayerName(tonumber(USER)) .. "^0 Er blevet sat til ^2 " .. tostring(GROUP)}
								})
							end
						end)

						TriggerClientEvent('chat:addMessage', Source, {
							args = {"", "Tilladelsesniveau ^2" .. GetPlayerName(tonumber(USER)) .. "^0 Er blevet sat til ^2 " .. tostring(GROUP)}
						})
					else
						TriggerClientEvent('chat:addMessage', Source, {
							args = {"", "Fejl!"}
			...
			elseif t == "bank" then
				...
				TriggerClientEvent('chat:addMessage', Source, {
					args = {"", "Superadmin har kun Adgang til dette!"}
				})
			end
		end)
	end)
...
RegisterCommand('Tilføjtilladelsesniveau', function(source, args, raw)
		...
		if (player and GetPlayerName(player)) then
			if level then
				TriggerEvent("es:setPlayerData", tonumber(args[1]), "permission_level", tonumber(args[2]), function(response, success)
					...
					TriggerClientEvent('chat:addMessage', -1, {
						args = {"", "Tilladelsesniveau ^2" .. GetPlayerName(tonumber(args[1])) .. "^0 Er blevet sat til ^2 " .. args[2]}
					})
				end)
			else
				RconPrint("Fejl!\n")
			...
			RconPrint("Spliller ikke ingame\n")
		end
	else
		RconPrint(": Tilføjgruppe [Spiller-id] [gruppe]\n")
	end
end, true)

RegisterCommand('Tilføjgruppe', function(source, args, raw)
	local player = tonumber(args[1])
	local group = args[2]
	if args[1] then
		if (player and GetPlayerName(player)) then
				...
				if(groups[args[2]])then
					TriggerEvent("es:getPlayerFromId", player, function(user)
						...
						TriggerEvent("es:setPlayerData", player, "group", args[2], function(response, success)
							...
							TriggerClientEvent('chat:addMessage', -1, {
								args = {"^1CONSOLE", "Ranken for ^2^*" .. GetPlayerName(player) .. "^r^0 er blevet sat til ^2^*" .. group}
							})
		...
		RconPrint("Usage: TilføjGruppe [spiller-id] [rank]\n")
	end
end, true)

RegisterCommand('Tilføjrolle', function(source, args, raw)
	local player = tonumber(args[1])
			...
			if args[2] then
				TriggerEvent("es:getPlayerFromId", player, function(user)
					...
					TriggerClientEvent('chat:addMessage', user.get('source'), {
						args = {"", "Du modtog ranket: ^2" .. role}
					})
				end)
			else
				RconPrint("Usage: Tillføjrolle [Spiller-id] [rolle]\n")
			end
		else
			RconPrint("Spiller ikke inde!\n")
		end
	else
		RconPrint("Usage: givrank [user-id] [role]\n")
	end
end, true)

RegisterCommand('Fjernrolle', function(source, args, raw)
	local player = tonumber(args[1])
	local role = table.concat(args, " ", 2)
	if args[1] then
		if (player and GetPlayerName(player)) then
			if args[2] then
				TriggerEvent("es:getPlayerFromId", tonumber(args[1]), function(user)
					...
					TriggerClientEvent('chat:addMessage', user.get('source'), {
						args = {"^1 ", "Dit rank blev frataget: ^2" .. role}
					})
				end)
			else
				RconPrint("Usage: FjernRolle [Spiller-id] [rolle]\n")
			end
		else
			RconPrint("Spiller ikke inde\n")
		end
	else
		RconPrint("Usage: fjernrolle [user-id] [role]\n")
	end
end, true)
...
TriggerEvent('es:addCommand', 'admin', function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, {
		args = {"", "Level: ^*^2 " .. tostring(user.get('permission_level'))}
	})
	TriggerClientEvent('chat:addMessage', source, {
		args = {"^1 ", "Group: ^*^2 " .. user.getGroup()}
	})
end, {help = "Viser hvilke rank du er."})
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				TriggerEvent('es:canGroupTarget', user.getGroup(), target.getGroup(), function(canTarget)
					if canTarget then
						...
						TriggerClientEvent('chat:addMessage', -1, {
							args = {"", "Player ^2" .. GetPlayerName(player) .. "^0 has been kicked(^2" .. reason .. "^0)"}
						})
						...
						TriggerClientEvent('chat:addMessage', source, { args = {"", "You can not target this person"}})
					end
				end)
			end)
		else
			TriggerClientEvent('chat:addMessage', source, { args = {"", "Incorrect player ID"}})
		end
	else
		TriggerClientEvent('chat:addMessage', source, { args = {"", "Incorrect player ID"}})
	end
...
TriggerEvent('es:addGroupCommand', 'noclip', "admin", function(source, args, user)
	...
	TriggerClientEvent('chat:addMessage', source, { args = {"", "Insufficienct permissions!"} })
	...
	if args[1] then
		if(tonumber(args[1]) and GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chat:addMessage', player, { args = {"^1 ", "You have been " .. state .. " by ^2" .. GetPlayerName(source)} })
				TriggerClientEvent('chat:addMessage', source, { args = {"^1 ", "Player ^2" .. GetPlayerName(player) .. "^0 has been " .. state} })
			end)
		else
			TriggerClientEvent('chat:addMessage', source, { args = {"^1 ", "Incorrect player ID"}})
		end
	else
		TriggerClientEvent('chat:addMessage', source, { args = {"^1 ", "Incorrect player ID"}})
	end
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = {"^1 ", "Insufficienct permissions!"} })
	...
	if args[1] then
		if(tonumber(args[1]) and GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chat:addMessage', player, { args = {"^1 ", "You have brought by ^2" .. GetPlayerName(source)} })
				TriggerClientEvent('chat:addMessage', source, { args = {"^1 ", "Player ^2" .. GetPlayerName(player) .. "^0 has been brought"} })
			end)
		else
			TriggerClientEvent('chat:addMessage', source, { args = {"^1 ", "Incorrect player ID"}})
		end
	...
	if args[1] then
		if(tonumber(args[1]) and GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chat:addMessage', player, { args = {"^1 ", "You have slapped by ^2" .. GetPlayerName(source)} })
				TriggerClientEvent('chat:addMessage', source, { args = {"^1 ", "Player ^2" .. GetPlayerName(player) .. "^0 has been slapped"} })
			end)
		else
			TriggerClientEvent('chat:addMessage', source, { args = {"^1 ", "Incorrect player ID"}})
		end
		...
		if(tonumber(args[1]) and GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				if(target)then
					...
					TriggerClientEvent('chat:addMessage', player, { args = {"^1 ", "You have been teleported to by ^2" .. GetPlayerName(source)} })
					TriggerClientEvent('chat:addMessage', source, { args = {"^1 ", "Teleported to player ^2" .. GetPlayerName(player) .. ""} })
				end
			end)
		else
			TriggerClientEvent('chat:addMessage', source, { args = {"^1 ", "Incorrect player ID"}})
...
TriggerEvent('es:addCommand', 'die', function(source, args, user)
	...
	TriggerClientEvent('chat:addMessage', source, { args = {"^1 ", "You killed yourself"} })
end, {help = "Suicide"})
	...
	if args[1] then
		if(tonumber(args[1]) and GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chat:addMessage', player, { args = {"^1 ", "You have been killed by ^2" .. GetPlayerName(source)} })
				TriggerClientEvent('chat:addMessage', source, { args = {"^1 ", "Player ^2" .. GetPlayerName(player) .. "^0 has been killed."} })
			end)
		else
			TriggerClientEvent('chat:addMessage', source, { args = {"^1 ", "Incorrect player ID"}})
		end
	...
	if args[1] then
		if(tonumber(args[1]) and GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				TriggerClientEvent('chat:addMessage', source, { args = {"^1 ", "Player ^2" .. GetPlayerName(player) .. "^0 has been crashed."} })
			end)
		else
			TriggerClientEvent('chat:addMessage', source, { args = {"^1 ", "Incorrect player ID"}})
		end
