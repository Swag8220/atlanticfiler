...
RegisterNetEvent("openvehicle")
AddEventHandler("openvehicle", function()
	openmenuvehicle()
end)
					...
					if ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'inventory') then
					  ...
					  if locked == 1 or class == 15 or class == 16 or class == 14 then
						  ...
						  TriggerServerEvent("esx_truck_inventory:getInventory", GetVehicleNumberPlateText(vehFront))
						   ...
						   ESX.ShowNotification('Bagagerummet er ~r~låst')
					  end
					end
				else
					ESX.ShowNotification('Ingen ~r~ køretøj ~w~ i nærheden')
				end
				lastOpen = true
				GUI.Time  = GetGameTimer()
			else
				TriggerEvent('esx:showNotification', "Nogen kigger allerede på bagagerummet.")
			end
		end, globalplate)
	end
	  ...
	  function(data, menu)
		  ...
		  if HasPedGotWeapon(playerPed,  weaponHash,  false) and weaponList[i].name ~= 'WEAPON_UNARMED' then
            ...
            if quantity > 0 and quantity <= tonumber(data3.current.count) and vehFront > 0  then
              ...
              --  VehicleMaxSpeed(closecar,totalweight,Config.VehicleLimit[GetVehicleClass(closecar)])
                ...
                ESX.ShowNotification('Vægt af bagagerummet : ~g~'.. Kgweight .. ' Kg / '..MaxVh..' Kg')
				Citizen.Wait(500)
				TriggerServerEvent("esx_truck_inventory:getInventory", GetVehicleNumberPlateText(vehFront))
              else
                ESX.ShowNotification('Du har nået grænsen på ~r~ '..MaxVh..' Kg')
              end
			else
				ESX.ShowNotification('~r~ Ugyldig mængde')
			end
		...
		elseif data.current.type == 'cancel' then
          ...
          --fin test
			...
			ESX.TriggerServerCallback('esx_truck:carry',function(test)
				if test == true then
				...
				ESX.ShowNotification('~r~ Du kan ikke bære mere')
				end
			end, item, count, itemtype)
			    else
			      ESX.ShowNotification('~r~ Ugyldig mængde')
			    end
