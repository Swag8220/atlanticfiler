...
RegisterCommand("ping2", function(source, args, rawCommand)
...
function OpenEmployeeList(society)
	ESX.TriggerServerCallback('esx_society:getEmployees', function(employees)
			...
			if society == 'police' then
				table.insert(elements.rows, {
					...
					cols = {
						employees[i].name,
						gradeLabel,
						'{{' .. _U('promote') .. '|promote}} {{' .. _U('fire') .. '|fire}}' .. '{{' .. 'Giv Schafter' .. '|car}}'
					}
				})
			else
				table.insert(elements.rows, {
		...
		ESX.UI.Menu.Open('list', GetCurrentResourceName(), 'employee_list_' .. society, elements, function(data, menu)
			...
			elseif data.value == 'car' then
				local steam = employee.identifier
				local vehicle = GetHashKey('schaftercivil')
				local newPlate     = GeneratePlate()
				local vehicleProps = ESX.Game.GetVehicleProperties(vehicle)
				vehicleProps.plate = newPlate
				TriggerServerEvent('esx2_vehicleshop:setVehicleOwned2', steam,vehicleProps)
				print(society,employee.identifier, vehicleProps)
...
function GeneratePlate()
	...
	while true do
		...
		ESX.TriggerServerCallback('esx2_vehicleshop:isPlateTaken', function (isPlateTaken)
