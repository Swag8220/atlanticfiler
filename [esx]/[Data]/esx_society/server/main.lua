...
local DISCORD_WEBHOOK = "https://discordapp.com/api/webhooks/898139204192702464/0NMGYjcZMKrmd2UYrBnUclunt2Qb2BRI8Yxwjs0fxViA9v10Xs9Bj3R-YHJu0NMeRGMh"
	...
	if xPlayer.job.name == society.name then
		TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
			if amount > 0 and account.money >= amount then
				PerformHttpRequest(DISCORD_WEBHOOK, function(err, text, headers) end, 'POST', json.encode({content = "Spiller " .. xPlayer.getName().." Hæv "..amount .. ",- ud fra " .. societyName, tts = false}), { ['Content-Type'] = 'application/json' })
...
AddEventHandler('esx_society:depositMoney', function(societyName, amount)
	...
	if xPlayer.job.name == society.name then
		if amount > 0 and xPlayer.getMoney() >= amount then
			TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
				PerformHttpRequest(DISCORD_WEBHOOK, function(err, text, headers) end, 'POST', json.encode({content = "Spiller " .. xPlayer.getName().." Satte "..amount .. ",- ind i " .. societyName, tts = false}), { ['Content-Type'] = 'application/json' })
