...
AddEventHandler('esx_carlock:sounds', function(soundFile, soundVolume)
	SendNUIMessage({
...
Citizen.CreateThread(function()
  ...
  while true do
	...
	if (IsControlJustPressed(1, 182)) then
		...
		notowned = 0
		if #cars == 0 then
			ESX.ShowNotification("Ingen biler at låse i nærheden.")
		else
			for j=1, #cars, 1 do
				...
				ESX.TriggerServerCallback('carlock:isVehicleOwner', function(owner)
					if owner and hasAlreadyLocked ~= true then
						...
						if lock == 1 or lock == 0 then
                            ...
                            PlayVehicleDoorCloseSound(carstrie[i], 0)
--							TriggerEvent("esx_carlock:sounds", "lock", 0.8)
							exports['mythic_notify']:DoHudText('inform', 'Du låste dit køretøj.', { ['background-color'] = '#ff0000', ['color'] = '#ffffff' })
						...
						elseif lock == 2 then
							...
							exports['mythic_notify']:DoHudText('inform', 'Du låste dit køretøj op.', { ['background-color'] = '#81ff7a', ['color'] = '#ffffff' })
							if not IsPedInAnyVehicle(PlayerPedId(), true) then
								TaskPlayAnim(PlayerPedId(), dict, "fob_click_fp", 8.0, 8.0, -1, 48, 1, false, false, false)
							end
							SetVehicleLights(carstrie[i], 2)
