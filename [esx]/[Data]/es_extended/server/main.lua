AddEventHandler('es:playerLoaded', function(source, _player)
	...
	TriggerEvent('es:getPlayerFromId', _source, function(player)
			...
			MySQL.Async.execute('UPDATE users SET name = @name WHERE identifier = @identifier', {
				...
				table.insert(tasks, function(cb)
					MySQL.Async.fetchAll('SELECT * FROM `users` WHERE `identifier` = @identifier', {
						...
						print(playerdata[1].firstname)
						table.insert(userData.rpname, {first = playerdata[1].firstname,last = playerdata[1].lastname})
						cb()
					end)
				end)
		...
		Async.parallel(tasks, function(results)
			local xPlayer = CreateExtendedPlayer(player, userData.accounts, userData.inventory, userData.job, userData.loadout, userData.playerName, userData.lastPosition, userData.rpname)
