function CreateExtendedPlayer(player, accounts, inventory, job, loadout, name, lastPosition, rpname)
	local self = {}
	self.rpname 	  = rpname
	self.player       = player
	...
	self.getAllMoney = function()
        return self.player.get('money')+self.player.get('bank')
    end
	...
	self.removeAllMoney = function(money)
        money = ESX.Math.Round(money)

        if money >= 0 then
            local wallet = self.player.get('money')
            if money >= wallet then
                self.player.removeMoney(wallet)
                self.player.removeBank(money-wallet)
            else
                self.player.removeMoney(money)
            end
        else
            print(('es_extended: %s attempted exploiting! (reason: player tried removing -1 all balance)'):format(self.identifier))
        end
    end
	...
	self.getRPname = function()
		return self.rpname[1].first .. " " .. self.rpname[1].last
	end

	self.getfirstname = function()
		return self.rpname[1].first
	end

	self.getlastname = function()
		return self.rpname[1].last
	end
