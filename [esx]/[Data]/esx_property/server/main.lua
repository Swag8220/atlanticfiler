...
local DISCORD_WEBHOOK = "https://discord.com/api/webhooks/894020570348392480/bqKt9DzY6PizpRQFdoqvEOlPzOEH2EbVe5nEBrzCHErOMT55lv18ZrLGhpS5l1ypg4sy"
...
AddEventHandler('esx_property:buyProperty', function(propertyName)
  ...
  if property.price <= xPlayer.get('money') then
    ...
    PerformHttpRequest(DISCORD_WEBHOOK, function(err, text, headers) end, 'POST', json.encode({content = steam.." købte en lejlighed til "..property.price .. "!", tts = false}), { ['Content-Type'] = 'application/json' })
...
AddEventHandler('esx_property:getItem', function(owner, type, item, count)
  ...
  local steam = GetPlayerName(_source)
	...
	if type == 'item_standard' then
		...
		TriggerEvent('esx_addoninventory:getInventory', 'property', xPlayerOwner.identifier, function(inventory)
          ...
          print("test")
          PerformHttpRequest(DISCORD_WEBHOOK, function(err, text, headers) end, 'POST', json.encode({content = steam.." tog "..count .. " ".. item .. " fra sit hus!", tts = false}), { ['Content-Type'] = 'application/json' })
  ...
  if type == 'item_weapon' then
    ...
    TriggerEvent('esx_datastore:getDataStore', 'property', xPlayerOwner.identifier, function(store)
      ...
      PerformHttpRequest(DISCORD_WEBHOOK, function(err, text, headers) end, 'POST', json.encode({content = steam.." tog "..weaponName.. " med ".. ammo .." skud ud af sit hus!", tts = false}), { ['Content-Type'] = 'application/json' })
...
AddEventHandler('esx_property:putItem', function(owner, type, item, count)
  ...
  if type == 'item_standard' then
    ...
    if playerItemCount >= count then
      ...
      TriggerEvent('esx_addoninventory:getInventory', 'property', xPlayerOwner.identifier, function(inventory)
        ...
        PerformHttpRequest(DISCORD_WEBHOOK, function(err, text, headers) end, 'POST', json.encode({content = steam.." lagde ".. count.. " " ..item.. " i sit hus!", tts = false}), { ['Content-Type'] = 'application/json' })
  ...
  if type == 'item_weapon' then
    ...
    TriggerEvent('esx_datastore:getDataStore', 'property', xPlayerOwner.identifier, function(store)
      ...
      PerformHttpRequest(DISCORD_WEBHOOK, function(err, text, headers) end, 'POST', json.encode({content = steam.." lagde " ..item.. " i sit hus!", tts = false}), { ['Content-Type'] = 'application/json' })
