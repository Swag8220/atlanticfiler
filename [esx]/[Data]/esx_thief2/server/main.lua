...
AddEventHandler('esx_thief:stealPlayerItem', function(target, itemType, itemName, amount)
	...
	if itemType == 'item_standard' then
		...
		if amount > 0 and targetItemCount >= amount then
			if not sourceXPlayer.canCarryItem(itemName, amount) then
				...
				targetXPlayer.showNotification(_U('someone_stole') .. ' ~r~x'  .. amount .. ' ' .. label )
				sourceXPlayer.showNotification(_U('you_stole') .. ' ~g~x' .. amount .. ' ' .. label .. ' ~w~' .. _U('from_your_target') )
			end
		else
	...
	elseif itemType == 'item_money' then
		if amount > 0 and targetXPlayer.get('money') >= amount then
			...
			targetXPlayer.showNotification(_U('someone_stole') .. ' ~r~$'  .. amount )
			sourceXPlayer.showNotification(_U('you_stole') .. ' ~g~$' .. amount .. ' ~w~' .. _U('from_your_target') )
		else
	...
	elseif itemType == 'item_account' then
		if amount > 0 and targetXPlayer.getAccount(itemName).money >= amount then
			...
			targetXPlayer.showNotification(_U('someone_stole') .. ' ~r~$'  .. amount .. ' ~w~' .. _U('of_black_money') )
			sourceXPlayer.showNotification(_U('you_stole') .. ' ~g~$' .. amount .. ' ~w~' .. _U('of_black_money') .. ' ' .. _U('from_your_target') )
		else
			sourceXPlayer.showNotification(_U('imp_invalid_amount'))
		end
	end
