...
function commonFunctions(value)
    if value == 'show_phonenumber' then
        if myPhoneNumber == '' then
            ESX.TriggerServerCallback('Atlantic:requestNumber', function(thisNumber)
    ...
    elseif value == 'show_id' then
        ...
        TriggerServerEvent('Atlantic-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(PlayerId()), 'driver')
        
        if distance ~= -1 and distance <= 3.0 then
            TriggerServerEvent('Atlantic-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(player), 'driver')
        end
    ...
    elseif value == 'surrender_now' then
        TriggerEvent('dpemotes:OnEmotePlay', 'Emotes', 'surrender')
    elseif value == 'hat_toggle' then
        TriggerEvent('Atlantic_animation:hat-toggle')
    elseif value == 'glasses_toggle' then
        TriggerEvent('Atlantic_animation:glasses-toggle')
    elseif value == 'mask_toggle' then
        TriggerEvent('Atlantic_animation:mask-toggle')
    elseif value == 'bag_toggle' then
        TriggerEvent('Atlantic_animation:bag-toggle')
    elseif value == 'shirt_toggle' then
        TriggerEvent('Atlantic_animation:shirt-toggle')
    elseif value == 'pants_toggle' then
        TriggerEvent('Atlantic_animation:pants-toggle')
    elseif value == 'shoes_toggle' then
        TriggerEvent('Atlantic_animation:shoes-toggle')
    elseif value == 'watch_toggle' then
        TriggerEvent('Atlantic_animation:watch-toggle')
    elseif value == 'chain_toggle' then
        TriggerEvent('Atlantic_animation:chain-toggle')
    elseif value == 'vest_toggle' then
        TriggerEvent('Atlantic_animation:vest-toggle')
    elseif value == 'decals_toggle' then
        TriggerEvent('Atlantic_animation:decals-toggle')
    end
end
    ...
    if closestPlayer ~= -1 and closestDistance <= 3.0 then
        if value == 'ziptie_on' then
            if not IsPedInAnyVehicle(GetPlayerPed(GetPlayerServerId(closestPlayer)), false) then
                TriggerServerEvent('Atlantic_ziptie:ziptie', GetPlayerServerId(closestPlayer))
        ...
        elseif value == 'ziptie_off' then
            if not IsPedInAnyVehicle(GetPlayerPed(GetPlayerServerId(closestPlayer)), false) then
                TriggerServerEvent('Atlantic_ziptie:RemoveZiptie', GetPlayerServerId(closestPlayer))
                --TriggerEvent('RunCheck', '')
            else
                exports['mythic_notify']:SendAlert('error', 'Du kan ikke tage strips af en der sider i en bil', 5000)
            end
        elseif value == 'put_vehicle' then
            TriggerServerEvent('Atlantic_ziptie:putInVehicle', GetPlayerServerId(closestPlayer))
            --TriggerEvent('RunCheck', '')
        elseif value == 'takeout_vehicle' then
            TriggerServerEvent('Atlantic_ziptie:OutVehicle', GetPlayerServerId(closestPlayer))
            --TriggerEvent('RunCheck', '')
        elseif value == 'drag_person' then
            TriggerServerEvent('Atlantic_ziptie:drag', GetPlayerServerId(closestPlayer))

            --TriggerEvent('RunCheck', 'drag')
        elseif value == 'check_pockets' then
            TriggerEvent('Atlantic_ziptie:checkPocket')
