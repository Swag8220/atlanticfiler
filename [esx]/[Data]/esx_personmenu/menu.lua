...
function AddPersonMenu(menu)
    disableMouse()
    local submenu = _menuPool:AddSubMenu(menu, "~h~~o~Personlige Funktioner", "", 1320, 0)
...
function AddVehiOutMenu(menu)
    ...
    if not IsPedInAnyVehicle(currentPlayer, false) then
        local submenu = _menuPool:AddSubMenu(menu, "~h~~o~Køretøjsmenu", "", 1320, 0)
        local Item = NativeUI.CreateItem("Åben/Luk Kølerhjelm", "")
        ...
        submenu:AddItem(Item)
        local Item = NativeUI.CreateItem("Åben/Luk Bagagerum", "")
        Item.Activated = function(ParentMenu, SelectedItem)
        ...
        local newitem = NativeUI.CreateListItem("Åben/Luk Dør", doors, 1)
        submenu:AddItem(newitem)
...
function AddCCMenu(menu)
    ...
    if IsPedInAnyVehicle(currentPlayer, false) then
        ...
        local newitem = NativeUI.CreateListItem("Sæt Fartgrænse", speedlimit, currentSpeed)
        ...
        menu.OnListSelect = function(sender, item, index)
            if item == newitem then
                ...
                if thisSpeedLimit == "Clear" then
                    currentSpeed = "1"
                    TriggerEvent('Atlantic-speedlimit:set', "clear")
                elseif thisSpeedLimit == "50" then
                    currentSpeed = "2"
                    TriggerEvent('Atlantic-speedlimit:set', 50)
                elseif thisSpeedLimit == "80" then
                    currentSpeed = "3"
                    TriggerEvent('Atlantic-speedlimit:set', 80)
                elseif thisSpeedLimit == "130" then
                    currentSpeed = "4"
                    TriggerEvent('Atlantic-speedlimit:set', 130)
                end
                Citizen.Wait(100)
            end
        end
...
function AddPersonInfoMenu(menu)
    disableMouse()
    local submenu = _menuPool:AddSubMenu(menu, "~h~~o~Personlig Information", "", 1320, 0)
    PlayerData = ESX.GetPlayerData()
    ...
    local Item = NativeUI.CreateItem("Kontanter: ~g~" .. PlayerData.money .. "~s~ DKK", "")
    submenu:AddItem(Item)
    local Item = NativeUI.CreateItem("Bank: ~g~" .. bankAccount .. "~s~ DKK", "")
    submenu:AddItem(Item)
    local Item = NativeUI.CreateItem("Sorte Penge: ~g~" .. blackmoneyAccount .. "~s~ DKK", "")
    submenu:AddItem(Item)
end
...
function AddAnimationsMenu(menu)
    ...
    local submenu = _menuPool:AddSubMenu(menu, "~h~~o~Animationer", "", 1320, 0)
    currentPlayer = PlayerPedId()
    if IsPedInAnyVehicle(currentPlayer, false) then
        for i = 1, #Config.CarAnimations, 1 do
            ...
            for j = 1, #Config.CarAnimations[i].items, 1 do
                ...
                Item.Activated = function(ParentMenu, SelectedItem)
                    ...
                    TriggerEvent('dpemotes:OnEmotePlay', Type, Name)
                    Citizen.Wait(100)
                end
