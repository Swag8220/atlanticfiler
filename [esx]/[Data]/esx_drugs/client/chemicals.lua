...
function OpenChemicalsMenu()
	...
	for k, v in pairs(ESX.GetPlayerData().inventory) do
		...
		if price and v.count >= 0 then
			table.insert(elements, {
				label = ('%s  <span style="color:green;">%s</span>'):format("Lav " .. v.label , ""),
				name = v.name,
			})
		end
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'ChemicalsConvertionMenu', {
		...
		menu.close()
		menuOpen = false
		TriggerServerEvent('esx_illegal:serverloggingChemicalsConvertionMenu', data.current.name, 1)
			...
			if IsControlJustReleased(0, Keys['E']) and not isPickingUp then
				...
				ESX.TriggerServerCallback('esx_illegal:canPickUp', function(canPickUp)
					...
					if canPickUp then
						...
						TriggerServerEvent('esx_illegal:serverloggingpickedUpChemicals')
					else
...
RegisterNetEvent('esx_illegal:serlog')
AddEventHandler('esx_illegal:serlog', function(xPlayer)
	TriggerServerEvent('esx_illegal:pickedUpChemicals')
end)

RegisterNetEvent('esx_illegal:serlog2')
AddEventHandler('esx_illegal:serlog2', function(xPlayer)
	TriggerServerEvent('esx_illegal:ChemicalsConvertionMenu')
end)
