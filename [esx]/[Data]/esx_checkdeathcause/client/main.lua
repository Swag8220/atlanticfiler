		...
		if not IsPedInAnyVehicle(GetPlayerPed(-1)) then
			...
			if closestPlayer ~= -1 and closestDistance < 3.0 then
				...
				if IsPedDeadOrDying(closestPed) then
					...
					DrawText3D(x,y,z, 'Tryk [~g~E~s~] for at tjekke personen', 0.4)
					
					if IsControlPressed(0,  Keys['E']) then
						OpenDeathMenu(closestPed)
					end
				end
			end
		end
...
function Notification(x,y,z)
	...
	while (timestamp + 4500) > GetGameTimer() do
		Citizen.Wait(0)
		DrawText3D(x, y, z, 'Skaden ser ud til at være forvoldt her', 0.4)
		checking = false
	end
end
...
function OpenDeathMenu(player)
	...
	table.insert(elements, {label = 'Forsøg at finde dødsårsagen', value = 'deathcause'})
	table.insert(elements, {label = 'Forsøg at finde skadepunktet', value = 'damage'})
	  ...
	  elements = elements,
		...
		if ac == 'damage' then
			...
			if success then
				...
				Notify('Det kunne ikke ses, hvor skaden blev forvoldt.')
			end
		end
		...
		if ac == 'deathcause' then
			...
			if checkArray(Melee, d) then
				Notify('Muligvis ramt af noget hårdt i hovedet')
			elseif checkArray(Bullet, d) then
				Notify('Skudsår indikerer død ved skud')
			elseif checkArray(Knife, d) then
				Notify('Stiksår indikerer død ved knivstik')
			elseif checkArray(Animal, d) then
				Notify('Bidmærker på kroppen indikerer død ved bid, muligvis fra et dyr.')
			elseif checkArray(FallDamage, d) then
				Notify('Højst sandsynlig død af fald, brækkede ben.')
			elseif checkArray(Explosion, d) then
				Notify('Eksplosionsdød, indikeret af den voldsomme skade.')
			elseif checkArray(Gas, d) then
				Notify('Højst sandsynligt død af en form for gas, lungerne er fyldte.')
			elseif checkArray(Burn, d) then
				Notify('Dødsårsagen er højst sandsynligt ild, der er brandsår overalt.')
			elseif checkArray(Drown, d) then
				Notify('Lungerne er fyldt med vand, indikerer druknedød.')
			elseif checkArray(Car, d) then
				Notify('Dødsårsagen er højst sandsynligt et biluheld.')
			else
				Notify('Ukendt dødsårsag')
			end
		end
