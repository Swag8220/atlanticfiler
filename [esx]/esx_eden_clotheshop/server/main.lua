...
AddEventHandler('esx_eden_clotheshop:pay', function()
	...
	xPlayer.removeAllMoney(Config.Price)
...
ESX.RegisterServerCallback('esx_eden_clotheshop:checkMoney', function(source, cb)
	...
	if xPlayer.getAllMoney() >= Config.Price then
		cb(true)
	else
		cb(false)
	end
...
ESX.RegisterServerCallback('esx_eden_clotheshop:getPlayerID', function(source, cb)
	local steam = GetPlayerIdentifier(source,0)
	cb(steam)
  end)


  ESX.RegisterServerCallback('esx_eden_clotheshop:getPlayerPed', function(source, cb)
	local identifier = ESX.GetPlayerFromId(source).getIdentifier()
	MySQL.Async.fetchAll('SELECT hash FROM playerpeds where identifier = @identifier', {['@identifier']=identifier}, function (result)
